...
class UserPostListTests(APITestCase):
    ...
    def test_post(self):
        alice_data = {"content": "Lorem ipsum dolor sit amet", "post_images": []}

        response = self.eve.post(self.url, alice_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

        response = self.bob.post(self.url, alice_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        response = self.alice.post(self.url, alice_data, format="json")
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
...
class UserPostDetailTests(APITestCase):
    def setUp(self):
        ...
        self.url = reverse("user_post_detail", kwargs={"pk": post.id})
...
class NewsfeedTests(APITestCase):
    ...
    def test_get(self):
        ...
        response = self.bob.get(url + "?limit=10&offset=10", format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data, [])

        response = self.bob.get(url + "?limit=10&offset=1", format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)

        response = self.alice.get(url + "?limit=10&offset=0", format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 1)
